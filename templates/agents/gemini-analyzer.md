...
```bash
gemini @{{MAIN_SRC_DIR}}/ -p "Analyze the main architecture patterns. Focus on: 1) Component structure 2) State management 3) API patterns. Be concise."
```
...
Quick health check:
```bash
gemini @package.json @{{MAIN_SRC_DIR}}/index.* -p "Quick check: Is this project properly configured and ready for development?"
```
...
gemini @{{MAIN_SRC_DIR}}/ -p "Can these changes be safely implemented: $CHANGES. Focus on conflicts and dependencies."
```
...
PATTERN="${2:-}"
DIRECTORY="${3:-{{MAIN_SRC_DIR}}}"

case "$CHECK_TYPE" in
    "initial")
        echo "🔍 Analyzing core architecture with Gemini..."
        gemini @{{MAIN_SRC_DIR}}/ -p "Analyze the main architecture patterns in this codebase. Focus on: 1) Component structure 2) State management 3) API patterns. Be concise and specific."
        ;;
    ...
    "quick-check")
        ...
        gemini @package.json @pyproject.toml @go.mod @Cargo.toml @{{MAIN_SRC_DIR}}/index.* @{{MAIN_SRC_DIR}}/main.* -p "Quick check: Is this project properly configured? Any obvious issues?"
        ;;

    "validate-feasibility")
        CHANGES="$PATTERN"  # Reuse parameter
        echo "🔍 Validating plan feasibility with Gemini..."
        gemini @{{MAIN_SRC_DIR}}/ -p "Can these changes be safely implemented without major refactoring: $CHANGES. Focus on potential conflicts and dependencies."
        ;;
