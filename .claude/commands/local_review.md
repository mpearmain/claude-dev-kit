...
3. **Set up the remote and worktree**:
    ...
    - If not, add it: `git remote add USERNAME git@github.com:USERNAME/claude-dev-kit`
    - Fetch from the remote: `git fetch USERNAME`
    - Create worktree: `git worktree add -b BRANCHNAME ~/wt/claude-dev-kit/SHORT_NAME USERNAME/BRANCHNAME`

4. **Configure the worktree**:
    - Copy Claude settings: `cp .claude/settings.local.json WORKTREE/.claude/`
    - Run setup: `make install`
    - Initialise thoughts (if using): `cd WORKTREE && npx humanlayer thoughts init --directory claude-dev-kit`
...
- Create worktree at `~/wt/claude-dev-kit/add-new-api` (or ticket number if found)
...
- **Setup command**: Adjust `make install` to match your project (npm install, poetry install, etc.)
- **Worktree location**: Change `~/wt/claude-dev-kit` if you prefer a different location
